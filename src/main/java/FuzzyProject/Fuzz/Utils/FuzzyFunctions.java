...
public class FuzzyFunctions {
    ...
    public static List<Double> calculateMembershipMatrix(Example example, List<SPFMiC> spfMiCS, int k, double m) {
        List<Double> memberships = new ArrayList<>();
        for(int i=0; i<spfMiCS.size(); i++) {
            memberships.add(calculateMembership(example, spfMiCS, i, k, m));
        }
        return memberships;
    }

    public static double calculateMembership(Example example, List<SPFMiC> spfMiCS, int indexSPFMiC, int k, double m) {
        double somatorio = 0;
        for(int i=0; i<spfMiCS.size(); i++) {
            double numerador = DistanceMeasures.calculaDistanciaEuclidiana(example, spfMiCS.get(indexSPFMiC).getCentroide());
            double denominador = DistanceMeasures.calculaDistanciaEuclidiana(example, spfMiCS.get(i).getCentroide());
            double pontencia = 2.0 / (m - 1.0);
            somatorio += Math.pow(numerador / denominador, pontencia);
        }
